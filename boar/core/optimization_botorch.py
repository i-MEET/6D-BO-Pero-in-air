...
class MooBOtorch(BoarOptimizer):
    ...
    def BoTorchOpti(self,n_jobs=[4,4], n_step_points = [5, 10], models=['Sobol','GPEI'], obj_type='MSE',loss='linear',threshold=1, model_kwargs_list = None, model_gen_kwargs_list = None,use_CUDA=True,is_MOO=False,use_custom_func=False,suggest_only=False,global_stopping_strategy=None,show_posterior=True,kwargs_posterior=None,verbose=True):
        ...
        if suggest_only and num_trials == 0:
            dum_model, dum_tkwargs, opt = self.get_model(estimator=models[i],use_CUDA=use_CUDA)
            generation_strategy = GenerationStrategy(steps=[GenerationStep(model=dum_model,num_trials=1,min_trials_observed=1,max_parallelism=1,model_kwargs = dum_tkwargs)]) # initialize the generation strategy
        else:
            generation_strategy = GenerationStrategy(steps=steps,) # initialize the generation strategy
        ...
        try:
            ...
            if not is_MOO and show_posterior:
                ...
                mean_target = mean_target/len(self.targets)
                Ys = np.asarray(Ys)
                triedX = ax_client.generation_strategy.trials_as_df
                ...
                sigma2 = np.sum((Ys-mean_target)**2)/(len(Ys)-1) # variance of the data
        ...
        if suggest_only and num_trials == 0:
            try:
                ...
                warnings.warn('We are in suggest_only mode and no trial was completed, could not fit the model, error message: '+str(e))
    ...
    def do_grid_posterior(self,step,fig,axes,gs,lb,ub,pf,beta_scaled, N, gpr, fscale, Nres, logscale, vmin, min_prob=1e-2, clear_axis=False,True_values=None,points=None):
        ...
        for comb in range(nb_params):
            ...
            # p = p/simps(simps(p,dims_GP[comb]),dims_GP[comb])
    ...
    def marginal_posterior_2D(self, x_name, y_name, pf = None, lb = None, ub = None, fig = None, ax = None, True_values = None, gpr = None, N = None, beta_scaled =None, fscale = None, Nres = None, Ninteg = 1e5, vmin = None, min_prob=None, points = None, logscale = False, show_plot = True, clear_axis = False, xlabel_pos = 'bottom', ylabel_pos = 'left', normalize = False, **kwargs):
        ...
        p = np.exp(lhlog-logsumexp(lhlog)) #normalize the likelihood
        # p = p/simps(simps(p,par_ay),par_ax) # normalize the likelihood
        p = p/np.sum(p) # normalize the likelihood
        if normalize:
            p = p/np.max(p) # normalize the likelihood for plotting
