...
class MultiObjectiveOptimizer(BoarOptimizer):
    ...
    def optimize_sko_parallel(self,n_jobs=4,n_yscale=20, n_BO=10, n_initial_points = 10,n_BO_warmstart=5,n_jobs_init=None,obj_type='MSE',loss='linear',threshold=1000,kwargs=None, base_estimator = 'GP',show_objective_func=True,kwargs_plot_obj=None,show_posterior=True,kwargs_posterior=None,verbose=True):
        ...
        # Using Equation 1.59 in Bishop
        mean_target = np.mean(Ys)
        sigma2 = np.sum((Ys-mean_target)**2)/(len(Ys)-1) # unbiased estimator of the variance
        beta = 1/sigma2 # precision = 1/sigma**2 
    ...
    def LH(self,X,beta_scaled,N,gpr,fscale):
        ...
        # From Bishop eq. 1.62
        # LLH = -beta/2 * SSE + N/2*np.log(beta) - N/2*np.log(2*np.pi) 
        # However, we will ignore the last term as it is a constant and does not influence the shape of the distribution but can make the integration harder duuring the marginalization
        LLH = -beta/2 * SSE 
    ...
    def do_grid_posterior(self,step,fig,axes,gs,lb,ub,pf,beta_scaled, N, gpr, fscale, Nres, logscale, vmin, min_prob=1e-2, clear_axis=False,True_values=None,points=None):
        ...
        for comb in list(itertools.combinations(range(nb_params), 2)):
            ...
            p = np.exp(lhlog-logsumexp(lhlog))
            p/=np.sum(p) # normalize the likelihood
            # p = p/simps(simps(p,dims_GP[d2]),dims_GP[d1]) # marginalize the likelihood
            p = p
            LHS = p/np.max(p) # normalize the likelihood for plotting
            if logscale:
                LHS = np.log10(LHS)
                LHS[LHS<vmin]=vmin
                vmin = vmin
        ...
        # ], N=256)
            ...
            axes[d2][d1].contourf(dims[d1],dims[d2],LHS.T,levels=50)#,vmin=vmin, vmax=vmax,levels=50)#,cmap=white_viridis)
            ...
            if step == 0 or clear_axis:
                if d1==0:
                    axes[d2][d1].set_ylabel(pnames_full[d2])
                ...
                if d2==nb_params-1:
                    axes[d2][d1].set_xlabel(pnames_full[d1])
        ...
        for comb in range(nb_params):
            ...
            # p = p/simps(p,dims_GP[comb]) # normalize the likelihood
            p = p/np.sum(p) # normalize the likelihood
            ...
            if comb==0:
                if logscale:
                    ...
                    axes[comb][comb].set_ylabel('Norm. P('+pnames_display[comb]+'|y)') 
                ...
                # set y labels on the right side
                axes[comb][comb].set_ylabel('Norm. P('+pnames_display[comb]+'|y)',labelpad=10)
                axes[comb][comb].yaxis.set_label_position("right")
            ...
            # m1 = np.sum(dims_GP[comb]*p) / np.sum(p) # the mean
            # m2 = np.sum(p*(dims_GP[comb]-m1)**2) / np.sum(p) # the variance
            # print('Mean of ',pnames[comb],' is ',m1,' and the standard deviation is ',np.sqrt(m2))
            # print('m1 {:.2e} m2 {:.2e}'.format(m1,np.sqrt(m2)))
            ...
            axes[comb][comb].fill_between(par_ax, LHS, where=mask, color='C0', alpha=0.5) # Fill the area below the curve between the vertical lines
        ...
        cbar.ax.set_ylabel('Norm. P(w|y)', rotation=90, va='bottom')
    ...
    def marginal_posterior_1D(self, x_name, pf = None, lb = None, ub = None, fig = None, ax = None, True_values = None, gpr = None, N = None, beta_scaled =None, fscale = None, Nres = None, Ninteg = 1e5, vmin = None, min_prob=None, points = None, logscale = False, show_plot = True, clear_axis = False, xlabel_pos = 'bottom', ylabel_pos = 'left', normalize = False, **kwargs):
        ...
        normalize : bool, optional
            if True we normalize the marginal posterior probability distribution, by default False
        '**kwargs' : dict, optional
        ...
        p = np.exp(lhlog-logsumexp(lhlog)) #normalize the likelihood
        # p = p/simps(p,par_ax) # normalize the likelihood
        p = p/np.sum(p) # normalize the likelihood
        p_not_norm = p
        if normalize:
            p = p/np.max(p) # normalize the likelihood
        ...
        sum_prob = 0
        for i in range(len(par_ax)-1):
            sum_prob += p_not_norm[i]
            if sum_prob >= 0.025:
                ilow = i
                break
        sum_prob = 0
        for i in range(len(par_ax)-1):
            sum_prob += p_not_norm[len(par_ax)-1-i]
            if sum_prob >= 0.025:
                ihigh = len(par_ax)-1-i
                break
        if ilow > idx_best:
        ...
        if ylabel_pos is not None:
            if normalize:
                ...
                norm_ = ''
            ax.set_ylabel(norm_+'P('+pnames_display[ii]+'|y)',position = ylabel_pos)
            if ylabel_pos == 'right':
    ...
    def marginal_posterior_2D(self, x_name, y_name, pf = None, lb = None, ub = None, fig = None, ax = None, True_values = None, gpr = None, N = None, beta_scaled =None, fscale = None, Nres = None, Ninteg = 1e5, vmin = None, min_prob=None, points = None, logscale = False, show_plot = True, clear_axis = False, xlabel_pos = 'bottom', ylabel_pos = 'left', normalize = False, **kwargs):
        ...
        p = np.exp(lhlog-logsumexp(lhlog)) #normalize the likelihood
        # p = p/simps(simps(p,par_ax),par_ay) # normalize the likelihood
        p = p/np.sum(p) # normalize the likelihood
        if normalize:
            p = p/np.max(p) # normalize the likelihood for plotting
        ...
        if normalize:
            ...
            vmin = 0
            vmax = np.max(p)
            contour_=ax.contourf(par_ax,par_ay,p.T,levels=50)
        ...
        if not_init:
            ...
            if logscale:
                ...
                if normalize:
                    ...
                    ticks = [10**vmin,10**(int(vmin/2)),10**vmax]
            else:
                norm = matplotlib.colors.Normalize(vmin=0, vmax=vmax)
                if normalize:
                    ticks = [0,0.5,1]
            ...
            if normalize:
                ...
                norm_ = ''
            cbar.ax.set_ylabel(norm_+'P('+pnames_display[iiX]+'&'+pnames_display[iiY]+'|y)', rotation=90, va='bottom',labelpad=15)
        ...
        for i in range(nb_params):
            for j in range(nb_params):
                if i==j: # plot the 1D posterior on the diagonal
                    ...
                    self.marginal_posterior_2D(pnames[j],pnames[i],pf=params,lb=lb_main,ub=ub_main,fig=fig,ax=axes[i,j],beta_scaled=beta_scaled,N=N,gpr=gpr,fscale=fscale,Nres=Nres,Ninteg=Ninteg,full_grid=full_grid,logscale=logscale,vmin=vmin,zoom=zoom,min_prob=min_prob,clear_axis=clear_axis,show_points=show_points,points=points,True_values=True_values,ylabel_pos=ylabel_pos,xlabel_pos=xlabel_pos,normallize=True)
    ...
    def single_point(self,X,y,params,n_jobs=4,base_estimator='GP',n_initial_points = 100,show_objective_func=True,kwargs_plot_obj=None,axis_type=[],show_posterior=True,kwargs_posterior=None):
        ...
        sigma2 = np.sum((y-mean_target)**2)/(len(y)-1) # unbiased estimator of the variance
        beta = 1/sigma2 # precision = 1/sigma**2 
